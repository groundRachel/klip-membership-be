...
public record MemberAuthentication(
        ...
        boolean isMember,
        @Schema(description = "파트너 가입 상태. 파트너이거나 Google 인증인 경우에만 노출.")
        SignUpStatus partnerStatus,
     ...
     * 인증 없음.
     */
    public static final MemberAuthentication NO_AUTH = new MemberAuthentication(false, false, null, null, emptyList());
    /**
     * 카카오 인증 (실질적으로 인증 없음)
     */
    public static final MemberAuthentication KAKAO = new MemberAuthentication(false, false, null, null, List.of(ROLE_KLIP_KAKAO));

    public static MemberAuthentication admin(@NonNull Admin admin, @NonNull AuthenticatedUser user) {
        return new MemberAuthentication(true, true, null, new Profile(admin), getAuthorities(user));
    }

    public static MemberAuthentication partner(@NonNull Partner partner, SignUpStatus partnerStatus, @NonNull AuthenticatedUser user) {
        return new MemberAuthentication(true, true, partnerStatus, new Profile(partner), getAuthorities(user));
    }

    public static MemberAuthentication google(SignUpStatus signUpStatus, AuthenticatedUser user) {
        return new MemberAuthentication(true, false, signUpStatus, null, getAuthorities(user));
    }
