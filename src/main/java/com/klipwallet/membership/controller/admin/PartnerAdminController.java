...
public class PartnerAdminController {
    ...
    @Operation(summary = "가입한 파트너 상세 조회")
    @ApiResponses({
            @ApiResponse(responseCode = "200", description = "조회 성공"),
            @ApiResponse(responseCode = "404", description = "존재하지 않는 ID", content = @Content(schema = @Schema(ref = "Error404")))
    })
    @GetMapping("/{partnerId}")
    public DetailByAdmin getPartnerDetail(
            @Parameter(description = "조회 할 파트너 ID", required = true, example = "3") @PathVariable Integer partnerId) {
