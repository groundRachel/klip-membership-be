...
class FaqToolControllerIntegrationTest {
    ...
    void listFaqWithDefaultQuery(@Autowired MockMvc mvc) throws Exception {
        ...
        mvc.perform(get("/tool/v1/faqs")
                            .contentType(APPLICATION_JSON))
           .andExpect(status().isOk())
           .andExpect(jsonPath("$.totalItem").value(4))
           ...
           .andExpect(jsonPath("$.items[0].creator.id").isNotEmpty())
           .andExpect(jsonPath("$.items[0].updater.id").isNotEmpty())
           .andExpect(jsonPath("$.items[0].status").value(Status.LIVE.toDisplay()))
           .andExpect(jsonPath("$.items[1].status").value(Status.LIVE.toDisplay()))
           .andExpect(jsonPath("$.items[2].status").value(Status.LIVE.toDisplay()))
           .andExpect(jsonPath("$.items[3].status").value(Status.LIVE.toDisplay()));
    }
